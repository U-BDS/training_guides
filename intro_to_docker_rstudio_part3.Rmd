...
In this helper script we have disabled a number of Singularity auto-mounts. Thus, you will need to pass in any directories to be mounted with `-b`

```bash
# here we mount $USER_DATA and the current working directory (along with tmp dir with --server-data-dir)
# Note they should be under a pair of quotes, and space separated

./run_rstudio_singularity.sh -s rstudio_4.0.4.sif -p "NBI" -u $USER -b "$USER_DATA `pwd`" --server-data-dir
```
...
<details><summary>Click here to expand __subset__ of `run_rstudio_singularity.sh`</summary>
...
mkdir -p run var-lib-rstudio-server #Solution_1
...
singularity exec \
    ...
    $add_bind_params \ 
    --bind run:/run,var-lib-rstudio-server:/var/lib/rstudio-server,database.conf:/etc/rstudio/database.conf \  #Solution_1
