...
<!-- #TODO: 1) address all open issues under this section which includes specific examples of the shell script we pushed --> 
<!-- #TODO: 2) add emphasis that it's best to run containers in separate working directories --> 
<!-- #TODO 3) Link https://uabrc.github.io/help/faq/#why-do-i-get-a-gray-screen-after-launching-my-ood-rstudio-server-session --> 
<!-- & mention this can be similarly observed with issues connect to "Unable to connect to service". The solution is the same (to clean `.rstudio`). Although this issue should be minimized by not mounting $HOME from the script --> 
