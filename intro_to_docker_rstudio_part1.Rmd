...
Note that the example above only uses a subset of Docker directives. For a full reference of directives see https://docs.docker.com/engine/reference/builder/. Also note that the data and script added to the image could have also been added via mounting - [shown in mounting section](# mounting-a-directory-to-the-container). This is usually best practice in many cases to keep the image size smaller, but for this example we include the small dataset/script to demonstrate two directives.

Now, that we have the Dockerfile ready, we build the image and run to launch the container - make sure you are in the directory that contains the Dockerfile for this example (in `./src/intro_to_docker_part1` of the [GitHub repository which contains this material](https://github.com/U-BDS/training_guides/tree/main/src)):
